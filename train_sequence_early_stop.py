"""
train_sequence_early_stop.py — 带早停的序列模型训练
"""
...
def train_model(variant, n=200, T=10, max_epochs=200, patience=20):
    """训练序列模型，带早停"""
    sparsity = 10
    ...
    print(f'  Best val_loss: {early_stopping.best_loss:.6f}')
...
print('='*60)
print('Experiment: Sequence Models with Early Stopping')
print('='*60)
...
# 训练序列模型
for variant in ['rnn', 'lstm', 'transformer']:
    print(f'\n--- {variant.upper()} ---')
    set_seed(42)
    ...
    print(f'  Test: error={np.mean(errors):.4f}, vs_ista={np.mean(ista_err)/(np.mean(errors)+1e-10):.2f}x')

# 保存结果
with open('sequence_models_early_stop_results.json', 'w') as f:
