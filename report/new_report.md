...
推理时间在 CPU 上测量，batch=32，T=10，n=200，取 50 次平均（经典方法 20 次）：

| 方法 | 推理时间 (ms/batch) | 相对速度 | 无噪声误差 | 参数效率 |
|------|-------------------|---------|-----------|---------|
| LISTA | **1.85** | **95× 快于 ISTA** | 0.743 | 600K 参数 |
| LISTA-CP | **1.69** | **104× 快于 ISTA** | 0.691 | 200K 参数 |
| LISTA-Momentum | 5.52 | 32× 快于 ISTA | 0.662 | 400K 参数 |
| DA-LISTA | 13.60 | 13× 快于 ISTA | **0.010** | 11.6K 参数 |
| RNN-LISTA | 22.83 | 8× 快于 ISTA | 0.047 | 4.5K 参数 |
| LSTM-LISTA | 27.13 | 6× 快于 ISTA | 0.050 | 5.6K 参数 |
| Transformer-LISTA | 157.74 | 1.1× 快于 ISTA | **0.016** | 5.4K 参数 |
| ISTA | 175.44 | 1.0× (基准) | 0.657 | 0 参数 |
| FISTA | 181.69 | 0.97× | 0.567 | 0 参数 |

**分析**：
- **LISTA/CP 推理最快**（~2ms），但精度最差——用参数量换速度，未换到精度
- **DA-LISTA 综合最优**：精度最高 (0.010)、参数最少 (11.6K)、推理适中 (13.6ms)
- **Transformer 精度优秀但推理最慢**（158ms），接近经典 ISTA 的速度，因自注意力的 $O(T^2)$ 复杂度
- 经典方法 (ISTA/FISTA) 最慢（~180ms），因 Python 循环逐样本求解
...
### 5.5 W 矩阵谱分析 (LISTA-Momentum)

| 层 | 与 I 的余弦相似度 | 谱半径 | 条件数 | 有效秩 |
|----|-----------------|--------|--------|--------|
| 1 | 0.999 | 1.027 | 1.24 | 200 |
| 5 | 0.998 | 1.059 | 1.26 | 200 |
| 10 | 0.998 | 1.146 | 1.31 | 200 |

![W 矩阵分析](W_matrix_analysis.png)

**分析**：
- **W 极度接近单位矩阵**：余弦相似度 0.998-0.999，说明学习到的梯度变换仅在 ISTA 基础上做微小调整
- **谱半径略大于 1** (1.03-1.15)，但网络稳定，说明谱半径 <1 是充分非必要条件
- **有效秩 = 200 (满秩)**：W 学习了完整的梯度变换，而非低秩近似
- **随层数增加，W 更偏离 I**：后期层需要更大的调整来处理残余误差
