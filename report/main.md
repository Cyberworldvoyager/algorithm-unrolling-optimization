...
| RNN-LISTA | 1.188 | 48,265 | 0.70× | 性能不佳 |
| LSTM-LISTA | 1.148 | 62,153 | 0.73× | 性能不佳 |
| Transformer-LISTA | 1.004 | 17,257 | 0.83× | 性能不佳 |
...
**分析**:
- 序列模型在当前配置下性能不如 ISTA 和 FISTA
- 可能原因：
  1. 训练不充分 (50 epochs 可能不够)
  2. 架构不适合此问题 (输入维度 2n 太大)
  3. 需要更多的训练数据
- 序列模型的优势在于可以处理任意迭代次数（动态展开），但需要更好的架构设计
